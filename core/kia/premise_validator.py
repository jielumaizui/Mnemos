...
PREMISE_VALIDATOR_DURATION_BUCKET_QUARTER_DAYS = 90
...
class PremiseValidator:
    ...
    def __init__(self, similarity_threshold: float = 0.3, time_sensitive_penalty: float = 0.2):
        self.similarity_threshold = similarity_threshold
        self.time_sensitive_penalty = time_sensitive_penalty

    def validate(self, premise: str, current_context: str, premise_date: str | None = None) -> Dict:
        """
        验证前提在当前上下文中是否仍然成立
        ...
        # 1. 语义匹配检查
        semantic_score, semantic_reason = self._check_semantic_match(premise, current_context)
        checks["semantic"] = {
        ...
        # 综合判定
        overall_score = semantic_score * 0.5 + time_score * 0.25 + obsolete_score * 0.25

        valid = (
            checks["semantic"]["passed"]
            and checks["timeliness"]["passed"]
            and checks["obsolete"]["passed"]
        )
    ...
    def _check_semantic_match(self, premise: str, current_context: str) -> Tuple[float, str]:
        ...
        if score >= self.similarity_threshold:
            return (
                score,
                f"语义匹配度 {score:.2f}（Jaccard: {jaccard:.2f}, 概念匹配: {len(matched_concepts)}）",
            )
        else:
            return score, f"语义匹配度不足 {score:.2f} < 阈值 {self.similarity_threshold}"

    def _check_timeliness(self, premise: str, premise_date: str | None = None) -> Tuple[float, str]:
        """时效性检查"""
        premise_lower = premise.lower()

        # 1. 检测时间敏感词汇
        time_sensitive_count = sum(
            1 for kw in self.TIME_SENSITIVE_KEYWORDS if kw.lower() in premise_lower
        )
        ...
        if premise_date:
            try:
                ...
                # 时间敏感的前提，90天后开始扣分
                if age_days > PREMISE_VALIDATOR_DURATION_BUCKET_QUARTER_DAYS:
                    age_penalty = min(
                        0.5, (age_days - PREMISE_VALIDATOR_DURATION_BUCKET_QUARTER_DAYS) / 360
                    )
            except ValueError:
        ...
        if score >= 0.5:
            ...
            return (
                score,
                f"时效性不足（时间敏感词: {time_sensitive_count}个, 年龄扣分: {age_penalty:.2f}）",
            )

    def _check_obsolete_signals(self, premise: str, current_context: str) -> Tuple[float, str]:
        """过时信号检查"""
        premise_lower = premise.lower()

        # 1. 前提中是否包含过时信号词
        obsolete_in_premise = sum(1 for kw in self.OBSOLETE_SIGNALS if kw.lower() in premise_lower)
    ...
    def _extract_core_concepts(text: str) -> List[str]:
        ...
        # 中文：XX技术/方法/框架/系统
        concepts = re.findall(
            r"[\u4e00-\u9fa5]{2,8}(?:技术|方法|框架|系统|工具|平台|语言|库|引擎|模型|算法|协议|模式|架构)",
            text,
        )
