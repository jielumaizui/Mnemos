...
def _detect_negation(sentence: str) -> bool:
    """检测是否是否定断言（反模式）"""
    for pattern in NEGATION_SIGNALS + AI_BEHAVIOR_ANTI_PATTERNS:
...
def _classify_form(assertions: List[Assertion], full_text: str) -> None:
    ...
    # 检测文本整体主题
    has_problem_solution = bool(
        re.search(r"(问题|bug|错误|故障|解决|修复|排查|根因|方案)", text_lower)
    )
    has_decision = bool(re.search(r"(选择|决定|决策|选了|用.+而不是|对比|优劣|权衡)", text_lower))
    has_methodology = bool(re.search(r"(框架|流程|步骤|方法论|模式|体系|标准|规范)", text_lower))
    bool(re.search(r"(原来|本质上|其实是|等价于|类似于|就像| analogy )", text_lower))

    for assertion in assertions:
        claim_lower = assertion.claim.lower()

        # AI 行为反模式（例如分析瘫痪）应直接进入 anti-pattern，避免被误归为普通问题分析。
        if any(
            re.search(pattern, assertion.claim, re.IGNORECASE)
            for pattern in AI_BEHAVIOR_ANTI_PATTERNS
        ):
        ...
        # 包含"选择/决定/采用...而非" → 决策记录
        if re.search(
            r"(选|选择|决定|定了|采用|使用).+(而非|不是|放弃|舍弃|优于|胜过|打败|替代|代替)",
            claim_lower,
        ):
        ...
        # 包含"步骤/流程/框架/方法是" → 方法论
        if re.search(
            r"(步骤|流程|框架|方法论|模式|方法)(是|为|：|:)\s*(先|第一步|首先|第\d+步)", claim_lower
        ):
        ...
        # 包含"如果...就/建议/优先" → 启发式
        if re.search(
            r"(如果|当|建议|推荐|优先|尽量|最好|优先).+(就|用|选|做|应该|可以)", claim_lower
        ):
            assertion.form = KnowledgeForm.HEURISTIC
            continue

        # 包含问题+解决 → 问题-解决对
        if has_problem_solution and re.search(
            r"(问题|bug|错误|故障|根因).+(解决|修复|方案|方法|原因是)", claim_lower
        ):
...
def merge_similar_assertions(
    assertions: List[Assertion], similarity_threshold: float = 0.85
) -> List[Assertion]:
    """
    合并相似的断言（简单的字符串包含检测）
    ...
    for a in sorted_a:
        ...
        for m in merged:
            ...
            m_tokens = _tokenize(m.claim)
            if _jaccard(a_tokens, m_tokens) > similarity_threshold:
                is_duplicate = True
                break
...
def main():
    ...
    logger.info("提取到 %s 条断言:\n", len(assertions))

    for i, a in enumerate(assertions, 1):
        logger.info("[%s] %s", i, a.form.value)
        logger.info("    断言: %s", a.claim)
        logger.info("    置信度: %.2f", a.confidence)
        if a.boundary_hint:
            logger.info("    边界: %s", a.boundary_hint)
        if a.is_negated:
            logger.info("    [否定断言]")
        logger.info()
