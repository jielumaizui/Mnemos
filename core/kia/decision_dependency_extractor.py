...
class DecisionGraph:
    ...
    def get_root_decisions(self) -> List[DecisionNode]:
        """获取无依赖的根决策"""
        dependent_ids = {edge[0] for edge in self.edges}
        dependent_ids.update(nid for nid, node in self.nodes.items() if node.dependencies)
        return [node for nid, node in self.nodes.items() if nid not in dependent_ids]
...
class DecisionDependencyExtractor:
    ...
    DECISION_PATTERNS = [
        # 中文决策模式
        re.compile(
            r"(?:我们?|团队|项目组)?(?:决定|决策|选择|确定|采用|使用|选用)\s*[:：]\s*(.+?)(?:[。；\n]|因为|由于|考虑到|基于|原因是)"
        ),
        re.compile(
            r"(?:最终|最后|综上|综合考虑后)?(?:决定|选择|确定|采用)\s+(.+?)(?:[。；\n]|因为|由于|考虑到)"
        ),
        re.compile(r"(?:方案|策略|架构|设计)\s*[:：]\s*(.+?)(?:[。；\n]|优势|缺点|风险)"),
        re.compile(
            r'(?:选用|采用|使用|引入|迁移到|切换到)\s*[`"]?(\w+[\w\s\-+.]*\w+)[`"]?(?:\s+作为|\s+用于|\s+因为)'
        ),
        # 英文决策模式
        re.compile(
            r"(?:we|team)?\s+(?:decided|chose|opted|selected)\s+(?:to\s+)?(.+?)(?:\.|because|since|due\s+to)",  # noqa: E501
            re.I,
        ),
    ...
    def _extract_contextual_premises(self, sentence: str, full_text: str) -> List[str]:
        """从上下文中提取关联前提"""
        premises = []  # type: ignore[var-annotated]
            ...
            for other_id, other in graph.nodes.items():
                ...
                for rel_type, keywords in self.DEPENDENCY_KEYWORDS.items():
                    for kw in keywords:
                        # 模式：当前决策 + 依赖词 + 其他决策
                        pattern = (
                            re.escape(node.decision[:20])
                            + r".{0,30}"
                            + re.escape(kw)
                            + r".{0,30}"
                            + re.escape(other.decision[:20])
                        )
