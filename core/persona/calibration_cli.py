...
from core.persona.pythia import PreferenceProfile  # noqa: E402
...
def run_calibration():
    ...
    logger.info("=" * 60)
    logger.info("当前画像版本: v%s，基于 %s 条信号", profile.version, profile.signal_count)
    logger.info("")
    logger.info("说明：")
...
def _calibrate_layer(
    layer_name: str, layer_key: str, profile: PreferenceProfile, calibration: Dict
):
    """校准一个层"""
    logger.info("\n%s", "=" * 40)
    ...
    for dim_key, (name, scale, labels) in dimensions.items():
        ...
        if dim_key in ins:
            logger.info("  [%s] -- 数据不足，跳过", name)
            calibration["ratings"][dim_key] = None
            continue

        logger.info("  [%s]", name)
        logger.info("  系统推断: %s (%.2f)", label, score)
        logger.info("  量表: %s", scale)

        while True:
            user_input = input("  你觉得这个推断准吗？(1-5，回车=跳过): ").strip()
            if not user_input:
                calibration["ratings"][dim_key] = None
                break
            try:
...
def _save_calibration(store: PersonaStore, profile: PreferenceProfile, calibration: Dict):
    """Commit one calibration, then replay its derived Persona projection."""
    ...
    confirmed_at = str(calibration["calibrated_at"])
    material_action = store.signal_store.prepare_persona_calibration_material_action(
        version=profile.version,
        confirmed_at=confirmed_at,
        calibration_score=avg_rating,
        calibration_metadata={
            "ratings": dict(calibration.get("ratings") or {}),
            "comments": dict(calibration.get("comments") or {}),
        },
        source_facts={
            "ratings": dict(calibration.get("ratings") or {}),
            "comments": dict(calibration.get("comments") or {}),
        ...
        evidence_refs=(
            ...
            *(f"calibration-rating:{key}" for key in sorted(ratings)),
        ),
    )
    if material_action is None:
        raise RuntimeError("persona calibration target was not found")
    stored = store.signal_store.record_persona_calibration(
        version=profile.version,
        confirmed_at=confirmed_at,
        calibration_score=avg_rating,
        calibration_metadata={
    ...
    versions = store.load_canonical_persona_versions_read_only(
        store.signal_store.db_path
    )
    ...
    committed = store.signal_store.get_latest_persona_version()
    if committed is None:
        raise RuntimeError("persona calibration committed without a canonical revision")
    calib_file = calib_dir / f"calibration-{committed['revision_id'].replace(':', '-')}.json"
    # trusted-scan: system_state owner=persona target=calibration_state expires=never persona calibration JSON state
    calib_file.write_text(json.dumps(calibration, ensure_ascii=False, indent=2), encoding="utf-8")

    # 删除已处理的挑战问题
    try:
        challenge_file = get_config().data_dir / "calibrations" / "pending_challenges.json"
        if challenge_file.exists():
            challenge_file.unlink()
    except (OSError, ValueError):
        logger.warning("Caught unexpected error", exc_info=True)
    logger.info("[校准] 结果已保存: %s", calib_file)
    logger.info(
        "[校准] 平均准确度: %.1f/5.0，画像置信度调整: %+.1f%%", avg_rating, confidence_delta
    )
...
def _print_calibration_report(calibration: Dict):
    ...
    logger.info("校准报告")
    logger.info("%s", "=" * 40)
    logger.info("  评分维度数: %s", len(ratings))
    logger.info("  平均准确度: %.1f/5.0", avg)
    ...
    if low_ratings:
        ...
        for dim, rating in low_ratings:
            ...
            logger.info("    - %s: %s/5 %s", dim, rating, comment and f"({comment})")
