...
DISTILLATION_PROMPT = '''你是一个知识蒸馏引擎（prompt_version: ''' + PROMPT_VERSION + '''）。你的任务是从一段 AI 与用户的对话中，提取出值得长期记录的知识，并生成结构化的 wiki 页面。
...
STAGE2_EXTRACT_PROMPT = '''你是一个知识提取引擎（prompt_version: ''' + PROMPT_VERSION + '''）。从以下对话中提取结构化的知识片段。

**内容清洗：**
1. 删除 [thinking] 块
2. 技术类知识保留代码块和最小可执行命令
3. 删除用户请求句（"帮我..."）
4. 删除 AI 执行流（"让我试一下..."）
