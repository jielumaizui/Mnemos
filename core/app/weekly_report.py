...
class WeeklyReportGenerator:
    ...
    def generate_weekly_report(self) -> str:
        ...
        reliable_sources = [source for source in sources if source["available"]]
        data_reliability = "db_backed" if reliable_sources else "unavailable"
        ...
        if reliable_sources:
            ...
            logger.warning("周报缺少可用数据源，跳过写入: %s", report_path)
        return content
    ...
    def _source_manifest(self) -> List[Dict[str, Any]]:
        ...
        specs = [
            ("wiki_state", database_dir / "wiki_state.db", "wiki_pages"),
            ("user_signals", database_dir / "user_signals.db", "session_signals"),
            ("sync_log", database_dir / "sync_log.db", "sync_log"),
        ]
        return [
            {
                "name": name,
                "path": str(path),
                "table": table,
                "available": self._table_exists(path, table),
            }
            for name, path, table in specs
        ]
    ...
    def _section_knowledge_growth(self) -> List[str]:
        ...
        try:
            wiki_state_db = get_config().database_dir / "wiki_state.db"
            total_pages = 0
            new_this_week = 0
            if wiki_state_db.exists():
                with sqlite3.connect(str(wiki_state_db), timeout=10) as conn:
                    ...
                    cursor = conn.execute(
                        "SELECT COUNT(*) FROM wiki_pages WHERE created_at >= datetime('now', '-7 days')"  # noqa: E501
                    )
    ...
    def _section_domain_shifts(self) -> List[str]:
        ...
        try:
            signals_db = get_config().database_dir / "user_signals.db"
            ...
            logger.warning("领域注意力数据采集失败", exc_info=True)
    ...
    def _section_blindspots(self) -> List[str]:
        ...
        try:
            metrics_db = get_config().database_dir / "wiki_metrics.db"
            no_result_queries = []
                ...
                try:
                    ...
                    if blindspots:
                        for bs in blindspots[:5]:
                            lines.append(
                                f"- **{bs.get('topic', '')}**：{bs.get('description', '')}"
                            )
                    else:
                        lines.append("- 本周未发现新盲点")
                except (ImportError, OSError, BlindspotAssetSchemaError):
                    lines.append("- 盲点检测未就绪")
        except (OSError, ValueError, sqlite3.Error):
            logger.warning("盲点检测失败", exc_info=True)
            lines.append("- 盲点检测未就绪")
    ...
    def _section_evolution_signals(self) -> List[str]:
        ...
        try:
            ...
            logger.warning("演化信号采集失败", exc_info=True)
    ...
    def _section_system_metrics(self) -> List[str]:
        ...
        try:
            ...
            sync_db = get_config().database_dir / "sync_log.db"
            if sync_db.exists():
                with sqlite3.connect(str(sync_db), timeout=10) as conn:
                    # 本周同步
                    cursor = conn.execute(
                        "SELECT COUNT(*) FROM sync_log WHERE synced_at >= datetime('now', '-7 days')"  # noqa: E501
                    )
    ...
    def _section_repeated_issues(self) -> List[str]:
        ...
        try:
            ...
            if sync_db.exists():
                with sqlite3.connect(str(sync_db), timeout=10) as conn:
                    cursor = conn.execute("""
                        ...
                        WHERE synced_at >= datetime('now', '-7 days')
                    """)
                    reasons = Counter()  # type: ignore[var-annotated]
            ...
            logger.warning("问题统计失败", exc_info=True)
        ...
        try:
            ...
            if sync_db.exists():
                with sqlite3.connect(str(sync_db), timeout=10) as conn:
                    ...
                    if cursor.fetchone()[0] >= 3:
                        suggestions.append(
                            "- sync_log 本周错误较多，建议检查 L1 storage API 连接和 Token 有效性"
                        )
            ...
            if wiki_state_db.exists():
                with sqlite3.connect(str(wiki_state_db), timeout=10) as conn:
                    ...
                    if cursor.fetchone()[0] < 2:
                        suggestions.append(
                            "- 本周 Wiki 新增页面较少，建议检查蒸馏流水线是否正常运转"
                        )
        except (OSError, ValueError, sqlite3.Error):
            logger.debug("行动建议生成失败", exc_info=True)
