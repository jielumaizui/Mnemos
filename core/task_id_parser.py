...
迁移自: L1 storage/task_id_parser.py
改造点:
...
TASK_ID_PARSER__CLEAN_KEYWORD_KEYWORD = 30
...
class TaskIdParser:
    ...
    def _clean_keyword(cls, keyword: str) -> str:
        ...
        return keyword[:TASK_ID_PARSER__CLEAN_KEYWORD_KEYWORD]  # 限制长度
...
class TagBuilder:
    ...
    def build_tags(
        ...
        model: str,  # 模型名
        task_id: str,  # 任务 ID
        scope: str = "restricted",  # 范围: restricted/private/project/explicit-public
        date: str | None = None,  # 日期 (YYYYMMDD)
    ) -> List[str]:
        """
        构建完整的五维标签
        ...
        2. time:{YYYYMMDD}
        3. model:{model_name}
        4. scope:{restricted|private|project|explicit-public}
