...
def main():
    if not DB_PATH.exists():
        ...
        raise SystemExit(
            1
        )  # [P2-FIX] Prefer raise SystemExit over sys.exit in non-top-level contexts
    ...
    with sqlite_conn(str(DB_PATH)) as conn:
        ...
        # 先删 aliases（因为外键未启用）
        cursor = conn.execute(
            f"DELETE FROM entity_aliases WHERE entity_uid IN (SELECT uid FROM entities WHERE name IN ({placeholders}))",  # noqa: E501  # nosec B608: internally generated ? placeholders
            names_to_delete,
        )
        ...
        # 再删 entities
        cursor = conn.execute(
            f"DELETE FROM entities WHERE name IN ({placeholders})",  # nosec B608: internally generated ? placeholders
            names_to_delete,
        )
