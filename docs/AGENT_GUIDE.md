...
| 上下文感知精准搜索 | `context_aware_search` | 画像加权 + 知识图谱召回 |
| 读取具体页面 | `wiki_read` | 读取 Wiki 页面 |
| 写入蒸馏结果 | `wiki_write` | 写入 Wiki 页面 |
| 意图路由分类 | `intent_route` | 自动分类用户意图 |
| 盲区检测 | `blindspot_check` | 检查知识库覆盖缺口 |
| 知识新鲜度 | `freshness_check` | 版本绑定 + 过时预警 |
| 预测性推送 | `predictive_push` | 基于上下文主动推荐 |
| 查看知识来源 | `knowledge_source_list` | 查看来源分布统计 |
| 查看复盘经验 | `retrospective_list` | 列出 retrospective 经验 |
...
由 `capture_turn`（每轮）+ `end_session`（结束）+ `knowledge_distill`（有价值时）自动完成。你不需要额外调用。
...
1. 检查 `capture_turn` 是否成功（确保 L1 已保存）
2. 重试一次
