...
def _write_page(path, frontmatter="", body="# Decision Flow\n1. 第一步\n2. 第二步\n"):
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(f"---\n{frontmatter}---\n{body}", encoding="utf-8")


def test_scan_wiki_for_cognitive_decision_assets_covers_entire_vault(tmp_path):
    from core.kia.ixion import CognitiveDecisionFlywheel

    page = tmp_path / "03-Tech" / "flow.md"
    _write_page(page, "类型: 方法论\n置信度: 0.9\n触发场景:\n  - 重构\n")
    flywheel = CognitiveDecisionFlywheel(wiki_base=str(tmp_path), db_path=str(tmp_path / "flywheel.db"))
    ...
    assert insights[0].direction == "wiki_to_cognitive_decision"
    assert insights[0].schema_version == "cognitive_decision_asset.v1"


def test_behavior_driven_repeated_tasks_generate_cognitive_decision_asset(tmp_path):
    from core.kia.ixion import CognitiveDecisionFlywheel
    ...
    assert insights[0].automation_derivative_allowed is False
    assert insights[0].auto_applicable is True


def test_skill_record_version_fields_and_history_are_persisted(tmp_path):
    from core.kia.ixion import AutomationSkillRecord, CognitiveDecisionFlywheel

    flywheel = CognitiveDecisionFlywheel(wiki_base=str(tmp_path), db_path=str(tmp_path / "flywheel.db"))
    flywheel.create_skill(
    ...
    with sqlite3.connect(tmp_path / "flywheel.db") as conn:
        assert (
            conn.execute(
                "SELECT COUNT(*) FROM skill_versions WHERE skill_name='RefactorHelper'"
            ).fetchone()[0]
            == 1
        )


def test_cleanup_deprecates_behavior_skills_and_archives(
    tmp_path,
):
    from core.kia.ixion import AutomationSkillRecord, CognitiveDecisionFlywheel
...
def test_run_cycle_uses_metis_fallback_when_persona_unavailable(
    tmp_path,
):
    from core.kia.ixion import CognitiveDecisionFlywheel

    _write_page(
        tmp_path / "03-Tech" / "a.md",
        "领域: 技术\n类型: 方法论\n复杂度: 入门\n置信度: 0.8\n时效性: 稳定\n创建日期: 2026-01-01\n",
    )
    result = CognitiveDecisionFlywheel(
