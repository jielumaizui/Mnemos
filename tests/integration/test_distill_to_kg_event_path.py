...
def mock_publish_event(monkeypatch):
    ...
    def _capture(event_type, agent, payload, *, trace_id="", subject_provenance=None):
        ...
        return trace_id or "distill-path-test-trace"

    monkeypatch.setattr("core.mnemos_bus.publish_event", _capture)
    return events


def test_emit_knowledge_distilled_publishes_once_without_direct_graph_writes(
    mock_publish_event, monkeypatch
):
    """Legacy document topology may publish once but cannot synchronously double-write."""
    ...
    kg_handler = MagicMock()
    cognitive_updater = MagicMock()
    monkeypatch.setattr(
        "core.kia.kg_event_handler.KGEventHandler",
        kg_handler,
    )
    monkeypatch.setattr(
        "core.cognitive_graph.updater.CognitiveGraphUpdater",
        cognitive_updater,
    )
    ...
    kg_handler.assert_not_called()
    cognitive_updater.assert_not_called()
