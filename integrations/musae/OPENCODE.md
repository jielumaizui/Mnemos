...
OpenCode 原生支持 MCP 协议。Mnemos 提供 MCP 服务器 `integrations/agora.py`。
...
```json
{
  "mcpServers": {
    "mnemos": {
      "command": "python3",
      "args": ["/path/to/integrations/agora.py"]
    }
  }
}
```
...
配置文件：`~/.opencode/settings.json` 中的 `hooks` 字段
...
### MCP 连接失败
1. 检查 `~/.opencode/settings.json` 中的 MCP 配置路径
2. 确认 `integrations/agora.py` 可执行
3. 测试：`python3 integrations/agora.py`
