...
- **紧急事件自动弹窗**：检测到高严重度事件（知识冲突、关键决策偏差、严重错误复现）时，自动打开 Obsidian 展示相关决策页面或复盘页面，确保你不会错过关键信号
- **定时复盘强制提醒**：用户预约的复盘时间到达时，自动打开 Obsidian 对应的复盘页面，不让你"忘了复盘"
